...
ClientConnection* find_client_by_socket(int socket) {
    lock_guard<mutex> lock(connections_mutex);
    for (auto &conn : connections) {
...
string get_user_name(int client_socket) {
    char buffer[BUFFER_SIZE];
...
int main() {
    ...
    if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR, &opt, sizeof(opt)) < 0) {
        cerr << "Erro ao configurar socket: " << strerror(errno) << endl;
        close(server_fd);
        return 1;
    }
    ...
    while (true) {
        ...
        if (new_socket < 0) {
            cerr << "Erro no accept: " << strerror(errno) << endl;
            continue;
        }
